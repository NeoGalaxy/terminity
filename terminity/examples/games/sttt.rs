...
use terminity_widgets::{
	frame, EventBubblingWidget, ResizableWisget, StructFrame, Widget, WidgetDisplay,
};
use Tile::*;
...
impl Game for SuperTTT {
	fn run(&self, out: &mut dyn io::Write) -> io::Result<()> {
		...
		let winner = loop {
			...
			match event::read()? {
				Event::Mouse(e) => {
					let _ = game_state.bubble_event(e);
				}
...
impl EventBubblingWidget for Tile {
	type HandledEvent = ();
